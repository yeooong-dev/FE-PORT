...
import { Info, NameEdit, Profile, Tab, TabContainer, TabTop, Wrap } from "./StMypage";
import { RiEditCircleFill } from "react-icons/ri";
import { imgAdd, imgGet, imgDelete, updatePassword, deleteAccount, updateName } from "../../api/mypage";
...
function Mypage() {
    ...
    const [selectedTab, setSelectedTab] = useState<"nameEdit" | "pwEdit" | "deleteAccount" | null>("nameEdit");
        ...
        if (user && user.id) {
            (async () => {
                try {
                    ...
                    if (imageUrl) {
                        imageUrl = imageUrl.replace("https://yeong-port.s3.ap-northeast-2.amazonaws.com/", "");
                        setCurrentImage("https://yeong-port.s3.ap-northeast-2.amazonaws.com/" + imageUrl);
                    } else {
                    ...
                    if (error.response && error.response.status === 404) {
                        ...
                        alert("프로필 이미지를 불러오는 데 실패했습니다. 다시 시도해주세요.");
                    }
                }
            })();
        }
    ...
    const handlePasswordChange = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!email || !newPassword || !confirmPassword || newPassword !== confirmPassword || !email) {
            setAlertType("error");
            setAlertMessage("모든 필드를 올바르게 입력해주세요.");
            return;
        }
        ...
        try {
            const response = await updatePassword(user.id, email, newPassword, confirmPassword);
            setAlertType("success");
            ...
            setAlertType("error");
            setAlertMessage("비밀번호 변경에 실패하였습니다. 정보를 다시 확인해주세요.");
        }
    };
    ...
    const handleAccountDelete = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (user === null || user === undefined) {
            setAlertType("error");
            setAlertMessage("유저 정보를 불러오는데 실패했습니다. 다시 시도해주세요.");
            return;
        }
