...
import { todoAdd, todoDelete, todoGet, todoToggleCheck, todoUpdate } from "../../api/todo";
...
function Todo() {
    ...
    const handleAddTodo = async () => {
        ...
        try {
            if (editTodoId !== null) {
                ...
                setTodos((prevTodos) => prevTodos.map((todo) => (todo.todo_id === editTodoId ? updatedTodo : todo)));
            } else {
    ...
    const handleToggleCheck = async (id: number) => {
        try {
            ...
            setTodos((prevTodos) => prevTodos.map((todo) => (todo.todo_id === id ? updatedTodo : todo)));
    ...
    useEffect(() => {
        async function fetchTodos() {
            try {
                ...
                const initialCheckedTodoIds = response.filter((todo) => todo.completed).map((todo) => todo.todo_id);
            ...
            <TodoWrap darkMode={darkMode}>
                ...
                <Todos darkMode={darkMode}>
                    {todos.length === 0 ? (
                        ...
                        todos.map((todo) => (
                            <div
                                className={`between ${checkedTodoIds.includes(todo.todo_id) ? "checked" : ""}`}
                                key={todo.todo_id}
                                ref={EndRef}
                            >
                                <div className='left'>
                                    <div className='check' onClick={() => handleToggleCheck(todo.todo_id)}>
                                        {checkedTodoIds.includes(todo.todo_id) ? (
                                            <BsCheckLg size='35' color='#3c57b3' />
                                        ) : null}
                                    </div>
                                    <span className={`text ${checkedTodoIds.includes(todo.todo_id) ? "checked" : ""}`}>
                                        {todo.text}
                                    </span>
                                </div>

                                <div className='right'>
                                    <span className={`time ${checkedTodoIds.includes(todo.todo_id) ? "checked" : ""}`}>
                                        {todo.time}
                                    </span>
                                    <BsPencil
                                        size='23'
...
const Todos = styled.div<darkProps>`
    ...
    .between {
        ...
        &:before {
            ...
            right: 0;
            bottom: 0;
            border-bottom: ${({ darkMode }) => (darkMode ? "1.5px solid #616161" : "1.5px solid #d6d6d6")};
        ...
        &.checked:before {
            ...
            left: 40px;
            right: 70px;
            border-bottom: ${({ darkMode }) => (darkMode ? "1.5px solid #616161" : "1.5px solid #e8e8e8")};
            color: #d6d6d6;
            width: 86%;
        }
